<template>
  <div class="container">
    ...
    <groupRoom :options="groupOptions" @updateList="updateList" @sendGroupInvitation="sendGroupInvitation"
               v-if="currentRoom == 'grp'"></groupRoom>
    ...
    <Group :group_id="groupInfooptions" v-if="currentRoom == 'group_info'" @sendGroupMessage="sendGroupMessage" @ShowinvitationFriend="ShowinvitationFriend"></Group>
  </div>
</template>
...
import chatRoom from './session/chatSession.vue'
import AIRoom from './session/bingSession.vue'
import groupRoom from './session/groupSession.vue'
import Info from './session/info.vue'
import Group from './session/groupInfo.vue'
...
export default {
  ...
  methods: {
     ...
     */
    initSocket() {
      this.socket = new WebSocket(`${this.wssaddress}/api/comfortTalker/auth/user_channel?username=${this.$store.getters.userInfo.username}`)
      //监听发来的请求
