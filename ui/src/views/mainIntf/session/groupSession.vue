    ...
    <div class="content">
      ...
      <div class="chat-content" ref="record">
        <div v-for="item, index in List" :key="index" class="chat-item">
          <span class="msg-create-time">{{ item.sendTime }}</span>
          <!--判断自己还是别人消息-->
          <div :class="item.sender_id == sender_id ? 'self' : 'other'">
            <!--展示头像-->
            <div class="avatar" :class="'avatar-' + item.type">
              <img :src="item.avatar ? getPath(item.avatar) : require('@/static/logo.png')" alt="" srcset=""
      ...
      <div class="send-msg" id="msg">
        <div class="send-msg-header">
          ...
          <input type="file" id="send-msg-header-img-file" accept="image/*,video/*" style="display: none"
                 @change="SendImage">
...
import { groupMemberList } from '@/api/grp';
export default {
  ...
  methods: {
    ...
    loadData() {
      groupMemberList({ group_id: this.receiver_id })
          .then((res) => {
            if (res.code == 200) {
    ...
    initSocket() {
      ...
      this.socket = new WebSocket(`${this.wssaddress}/api/comfortTalker/msg/chat?room=${this.room}&id=${this.receiver_id}&type=grp`)
      this.socket.onopen = () => {
        console.log('连接成功');
      };
