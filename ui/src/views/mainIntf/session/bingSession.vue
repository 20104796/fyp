...
export default {
  ...
  data() {
    return {
      externalUrl: 'https://www.bing.com/chat?q=Microsoft+Copilot&FORM=hpcodx',
  ...
  mounted() {
    ...
    if (this.options.total == this.options.us_count) {
      //this.$msg.warning("你的ai对话已经全部用完,无法继续使用")
  ...
  methods: {
            ...
            if (response.arguments[0].messages) {
              if (!this.chatList[this.current]) {
                ...
                this.chatList[this.current] = {
                  avatar: require("@/static/chat/Bing.svg"),
                  sender_id: 0,
                  type: 'text',
            ...
            try {
              if (response.item.messages.length >= 2) {
                if (this.invocationId == 0) {
                  ...
                  this.disabeldSend = false
                  if (this.resetFlag) {
                    this.$msg.warning("当前会话已结束,即将重新创建新会话!!!")
                    this.lockScreen = this.$loading({
                      target: document.getElementById("AI"),
                      lock: true,
                      text: '正在建立新会话',
    ...
    initSocket() {
      ...
      this.socket = new WebSocket(`${this.wssaddress}/api/comfortTalker/bing/contect?room=${this.room}`)
      this.socket.onopen = () => {
        console.log('连接成功');
      };
    ...
    getPath(item) {
      ...
      return require("@/static/chat/Bing.svg")
