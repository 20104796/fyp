<template>
  <div class="chat-container" @click="showEmoji = false" :style="containerAni">
    ...
    <div class="header">
      <div class="header-top">
        <div class="header-left">
          <!--   <img src="@/static/logo.png" alt="" width="15px"> -->
        </div>
        <div class="header-right">
          <img src="@/static/chat/minus.png" alt="" width="20px" class="header-icon" @click="minWindow">
          <img src="@/static/chat/max.png" alt="" width="12px" class="header-icon" @click="maxWindow">
          <img src="@/static/chat/close.png" alt="" width="20px" class="header-icon" @click="closeWindow">
        </div>
      </div>
    ...
    <div class="not-to-choose" v-if="empty">
      <img src="@/static/logo.png" alt="" width="160">
      <h3 style="margin-top: 40px;">Stay hungry. Stay foolish.</h3>
      <span style="color: #838384;font-size: 13px;margin-top: 10px;">Everything is possible</span>
    </div>

    <!-- list 主界面的消息展示 + 下部分的发送消息 -->
    <div class="content">
      <!-- list 主界面的消息展示-->
      <div class="chat-content" ref="record">
        <div v-for="item, index in List" :key="index" class="chat-item">
          <!-- 时间 展示 -->
          <span class="msg-create-time" v-if="item.sendTime">{{item.sendTime }}</span>
          <div :class="item.sender_id == sender_id ? 'self' : 'other'">
            <!--设置信息格式-->
            <!--头像-->
            <div class="avatar" :class="'avatar-' + item.type">
              <img :src="item.avatar ? getPath(item.avatar) : require('@/static/logo.png')" alt="" srcset="" width="45px" height="45px">
            </div>
            <!--消息 分不同种类的消息-->
            <div class="aChat" style="white-space: pre-wrap;" :class="item.type">
              ...
              <div v-else-if="item.type == 'video'">
                ...
                <div class="play-btn" @click="showVideoDialog = true, currentVideo = getPath(item.content)">
                  <img src="@/static/chat/play.png" alt="" width="45">
                </div>
              </div>
              <!-- 文件-->
      ...
      <!-- list 发送消息-->
      <div class="send-msg" id="msg">
        <!--发送特殊消息-->
        <div class="send-msg-header">
          <img src="@/static/chat/icon.png" alt="" @click.stop="showEmoji = !showEmoji">
          <img src="@/static/chat/photo.png" alt="" @click="choosePhoto">
          <img src="@/static/chat/folder.png" alt="" @click="chooseFile">
          <img src="@/static/chat/audio.png" alt="" @click="SendAudioInvitation">
          <img src="@/static/chat/video.png" alt="" @click="SendVideoInvitation">
          <input type="file" id="send-msg-header-img-file" accept="image/*,video/*" style="display: none" @change="SendImage">
          <input type="file" id="send-msg-header-file" accept="*" style="display: none" @change="SendFile">
        </div>

        <!--发送text消息-->
        <div class="send-msg-content">
          <textarea class="send-msg-content-textarea" v-model="content" @keyup="onKeyUp" />
        </div>

        <!-- 发送按钮-->
        <div class="send-msg-btn">
          <el-dropdown split-button type="primary" trigger="click" style="height: 30px;" @click="sendMessage">
            <div style="width: 100%;">Send</div>
          </el-dropdown>
        </div>
        <!-- 表情选项-->
    ...
    <!--为了展示视频的界面-->
    <el-dialog title="Video" :visible.sync="showVideoDialog" width="60vw">
...
export default {
  ...
  methods: {
    ...
    initSocket() {
      ...
      this.socket = new WebSocket(`${this.wssaddress}/api/comfortTalker/msg/chat?room=${this.room}&id=${this.sender_id}&type=private`)
      this.socket.onopen = () => {
        console.log('连接成功');
      };
    ...
    //选择图片
    choosePhoto() {
      var choose = document.getElementById('send-msg-header-img-file')
      choose.click()
    },
    SendImage(e) {
    ...
    //选择文件
    chooseFile() {
      var choose = document.getElementById('send-msg-header-file')
      choose.click()
    },
    //发送文件
    async SendFile(e) {
