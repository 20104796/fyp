  ...
  <div class="audio-bg">
    ...
    <div class="audio-content">
      ...
      <div class="audio-choses" v-if="!flag">
        ...
        <div style="color: white;">You received grp video!!!</div>
        <div>
          <img src="../../static/chat/accept.png" alt="" width="50" @click="acceptAudio">
      ...
      <div class="audio-accept" v-if="flag">
        <div class="audio-headers">
          <div class="audio-avatar" v-for="item, index in userList" :key="index"
               @mouseover="mouseOverHandler(item.name)" @mouseleave="currentVideo = ''">
            <div :id="'loading_' + item.name" v-if="!item.accept">
              <img :src="item.avatar ? getAvatarPath(item.avatar) : require('@/static/black.png')" alt=""
                   style="width: 100%;height: 100%;object-fit: contain;object-position: center;">
            </div>
            <div class="video-max" v-if="item.accept && currentVideo == item.name"
            >
              <img src="@/static/full.png" alt="" width="25" @click="fullScreen('video_' + item.name)">
            </div>
            <video :id="'video_' + item.name" autoplay style="width: 100%;height: 100%;object-position: center;"
                   :poster="item.avatar ? getAvatarPath(item.avatar) : require('@/static/black.png')"
...
import { GetRTCUser } from '@/api/grp';
export default {
  data() {
    return {
      flag: false,
  ...
  methods: {
    ...
    //初始化websocket
    initSocket() {
      this.socket = new WebSocket(`${this.wssaddress}/api/comfortTalker/rtc/grp?room=${this.room}&username=${this.$store.getters.userInfo.username}`)
    ...
    async acceptAudio() {
      ...
      try {
        ...
        try {
          ...
          alert("Microphone not supported. Adjust settings and try again.")
          this.socket.send(JSON.stringify({
            name: "reject",
          }))
          this.socket.close()
    ...
    rejectAudio() {
      if (this.socket) {
        ...
        this.socket.close()
        this.socket = null
        this.$msg.warning("You have hang up. Exiting soon")
        // setTimeout(() => {
        //     win.close()
        // }, 1000);
      }
