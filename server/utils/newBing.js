...
let HEADERS = {
    authority: 'https://www.bing.com/chat?q=Microsoft+Copilot&FORM=hpcodx',
    accept:
        ...
        request.get(options, (error, response, body) => {
            if (!error && response.statusCode == 200) {
                ...
                if (result.result.value != 'Success') {
                    resolve({ code: 4009, message: "Please login new bing in edge" })
                } else {
                ...
                resolve({ code: 4010, message: "No proxy" })
            }
        });
    })
