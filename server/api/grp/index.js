...
let { RespParamErr, RespServerErr, RespExitFriendErr, RespUpdateErr, RespCreateErr, RespExitGroupErr, RespGroupInsertError, RespGroupDeletError } = require('../../handler/error');
...
async function JoinGroupChat(req, res) {
    ...
    // 查询数据失败
    if (err) return RespError(res, RespServerErr)
    sql = 'select session from group_chat where id=?'
...
async function DeleteUserFromGroup(req, res) {
    ...
    let sql = "select creator_id,session from group_chat where id=?"
    let { err, results } = await Query(sql, [group_id])
    // 查询数据失败
    if (err) return RespError(res, RespServerErr)
    if (results.length == 0) {
    ...
    if (results[0].creator_id == user_id) {
        ...
        //删除相对于的所有聊天记录
        sql = 'DELETE FROM msg WHERE type="grp" and session=?'
        ...
        //删除相对于的所有聊天记录
        sql = 'DELETE FROM msg WHERE type="grp" and session=? and (sender_id = ? OR receiver_id= ?)'
