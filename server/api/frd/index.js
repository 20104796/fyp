...
async function getFriendInfo(req, res) {
    ...
    let sql = 'SELECT user_id,session,user.username,user.signature,user.avatar,user.name,remark,group_id FROM frd,user WHERE group_id=? and user_id=? and user.id=?'
...
async function updateFriendInfo(req, res) {
    ...
    let sql = 'update frd set group_id=?,remark=? where user_id=? and group_id=?'
...
async function getFriendList(group_id) {
    const sql = 'select * from frd where group_id=?'
    let { results } = await Query(sql, [group_id])
    return results
}
//添加好友
async function addFriend(friendInfo) {
    const sqlStr = 'insert into frd  set ?'
