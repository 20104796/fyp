...
let { RespParamErr, RespServerErr, RespExitFriendErr, RespUpdateErr, RespCreateErr } = require('../../handler/error');
const { RespError, RespSuccess, RespData } = require('../../handler/resp');
 ...
 * 
 * 群聊历史记录获取
 * 需要获取的字段 avatar,lastMessage(最后一条消息),name,session,type,unreadCount(当前用户未读的消息数量),updated_at,group_id(分组id)
 * 2.根据group_chat获取头像,name,session,group_id
 * 
 */
async function List(req, res) {
    ...
    sql = 'SELECT gc.id as group_id,avatar,name,gc.session,msg_sta.updated_at FROM group_chat as gc,(SELECT * FROM group_members WHERE user_id=?) as gm,message_statistics as msg_sta  WHERE gc.id=gm.group_id and gc.session=msg_sta.session  ORDER BY msg_sta.updated_at DESC;'
    ...
    for (const index in results2) {
        ...
        // sql = `SELECT count(*) as unreadCount FROM msg WHERE session=? and receiver_id=? and status=0`
        // let r = await Query(sql, [item.session, id])
...
async function ChatConnect(ws, req) {
    ...
    let resp
    if (type == 'group') {
        sql = 'SELECT gm.nickname,m.*,u.avatar FROM (SELECT sender_id, receiver_id, content, session, media_type,msg.created_at FROM msg WHERE `session` =? AND `type` = ?) AS m LEFT JOIN user as u ON u.`id`=m.`sender_id` LEFT JOIN group_members as gm on gm.group_id=? and user_id=u.`id` ORDER BY created_at ASC'
        resp = await Query(sql, [room, type, id])
    } else {
        sql = 'SELECT m.*,u.avatar FROM (SELECT sender_id, receiver_id, content, session, media_type,msg.created_at FROM msg WHERE `session` =? AND `type` = ?  ORDER BY created_at ASC) AS m LEFT JOIN user as u ON u.`id`=m.`sender_id`'
    ...
    //将所有未读消息变成已读
    sql = 'update msg set status=1 where receiver_id=? and session=? and type=? and status=0'
